...
describe 'CQ JAR file' do
  ...
  it 'contains proper content' do
    expect(
      command('grep -i adobe /opt/cq/publish/cq-quickstart-6.0.0.jar').stdout
    ).to match('matches')
  end

  it 'is owned by cq' do
    expect(file('/opt/cq/publish/cq-quickstart-6.0.0.jar')).to be_owned_by('cq')
  end

  it 'is grouped into cq' do
    expect(
      file('/opt/cq/publish/cq-quickstart-6.0.0.jar')
...
describe 'CQ init script' do
  it 'exists' do
    expect(file('/etc/init.d/cq60-publish')).to be_file
  end

  it 'is executable' do
    expect(file('/etc/init.d/cq60-publish')).to be_executable
  end

  it 'is owned by root' do
    expect(file('/etc/init.d/cq60-publish')).to be_owned_by('root')
  end

  it 'is grouped into root' do
    expect(file('/etc/init.d/cq60-publish')).to be_grouped_into('root')
  end
...
describe 'CQ publish config file' do
  it 'exists' do
    expect(
      file('/opt/cq/publish/crx-quickstart/conf/cq60-publish.conf')
    ).to be_file
  end
...
describe 'CQ publish service' do
  it 'is running' do
    expect(service('cq60-publish')).to be_running
  end

  it 'is enabled' do
    expect(service('cq60-publish')).to be_enabled
  end
end
