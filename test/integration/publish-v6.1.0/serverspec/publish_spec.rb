...
describe 'CQ JAR file' do
  ...
  it 'contains proper content' do
    expect(
      command('grep -i adobe /opt/cq/publish/cq-quickstart-6.1.0.jar').stdout
    ).to match('matches')
  end

  it 'is owned by cq' do
    expect(file('/opt/cq/publish/cq-quickstart-6.1.0.jar')).to be_owned_by('cq')
  end

  it 'is grouped into cq' do
    expect(
      file('/opt/cq/publish/cq-quickstart-6.1.0.jar')
...
describe 'CQ init script' do
  it 'exists' do
    expect(file('/etc/init.d/cq61-publish')).to be_file
  end

  it 'is executable' do
    expect(file('/etc/init.d/cq61-publish')).to be_executable
  end

  it 'is owned by root' do
    expect(file('/etc/init.d/cq61-publish')).to be_owned_by('root')
  end

  it 'is grouped into root' do
    expect(file('/etc/init.d/cq61-publish')).to be_grouped_into('root')
  end

  it 'contains PID_DIR line' do
    expect(
      file('/etc/init.d/cq61-publish').content
  ...
  it 'contains CQ_CONF_FILE variable for publish' do
    expect(
      file('/etc/init.d/cq61-publish').content
    ).to match(
      'CQ_CONF_FILE=/opt/cq/publish/crx-quickstart/conf/cq61-publish.conf')
  end
  ...
  it 'does not contain CQ_CONF_FILE variable for author' do
    expect(
      ...
      'CQ_CONF_FILE=/opt/cq/publish/crx-quickstart/conf/cq61-author.conf')
  end
end
...
describe 'CQ publish config file' do
  it 'exists' do
    expect(
      file('/opt/cq/publish/crx-quickstart/conf/cq61-publish.conf')
    ).to be_file
  end
  ...
  it 'contains valid content' do
    expect(
      file('/opt/cq/publish/crx-quickstart/conf/cq61-publish.conf').content
...
describe 'CQ publish service' do
  it 'is running' do
    expect(service('cq61-publish')).to be_running
  end

  it 'is enabled' do
    expect(service('cq61-publish')).to be_enabled
  end
end
